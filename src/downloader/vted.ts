...
export async function login(browser: BrowserContext, email: string, password: string)
{
    ...
    if (page.url() === "https://account.vted.vn/Account/Login")
        cookies = null;
    else
...
export async function logout(browser: BrowserContext)
{
    ...
    await page.$eval("form#logoutForm", el => el.submit());
    await page.close();
}
...
async function downloadLesson(browser: BrowserContext, link: string, output: string)
{
    ...
    const title = await page.$eval(".lesson-detail h4", el => el.textContent!.trim());
    const videoLinks = [
        link,
        ...await page.$$eval("a.btn[href^='/khoa-hoc/baigiang']", elems => elems.map(el => el.href))
    ];
    ...
    const videoInfo = await pMap(videoLinks, async (link) => {
        ...
        const title = await page.$eval("input[type='button'][disabled]", el => el.value).catch(() => "video");
        
        const videoFrameUrl = await page.$eval(".asyncVideo", el => "https://vted.vn" + el.getAttribute("data-url"));
        await page.goto(videoFrameUrl, { waitUntil: "domcontentloaded" });
        const videoLink = await page.$eval("video", el => el.src);
        await page.close();
        ++finished;
...
async function downloadExam(browser: BrowserContext, link: string, output: string)
{
    ...
    const title = await page.$eval(".lesson-detail-header h2", el => el.textContent!.trim());
    const subdir = join(output, sanitizePath(title));
    await mkdir(subdir, { recursive: true }).catch(() => {});

    const answerLink = await page.$eval("a[href^='/practice/practiceresult']", el => el.href);
    ...
    if (pdfAvailable)
    {
        ...
        const pdfLink = await page.$eval("a[href^='/practice/download']", el => el.href);
        await got(pdfLink, {
            cookieJar: await toToughCookieJar(await page.cookies(), "https://vted.vn")
        }).buffer().then(buf => writeFile(join(subdir, "Đề bài.pdf"), buf));
    }

    spinner.text = "Fetching answers...";
    await page.$$eval(".panel-body .answer", elems => elems.forEach(e => e.remove()));
    ...
    {
        ++i;
        spinner.text = `Capturing question ${i}/${answers.length} (${(i * 100/answers.length).toFixed(0)}%)...`;
        await answer.evaluate(e => {
            e.classList.remove("lock");
            e.classList.add("block");
        });
        await answer.$eval(".explain-question a", el => el.click());
