...
export const website = "nap.edu.vn";
...
{
    const {code, data} = await got.post("https://api.nap.edu.vn/auth/signin", {
        json: { email, password }
    }).json<AuthResult>();
...
async function downloadExam(token: string, id: string, output: string)
{
    ...
    const { code, data, message } = await got.post("https://api.nap.edu.vn/testing/detail", {
        json: { id },
        headers
    }).json<Exam>();
    if (code !== 200)
    ...
    const videoLinks = await pMap(questions, async ({_id: id, question}, i) => {
        ...
        const { code, data } = await got.post("https://api.nap.edu.vn/question/answer", {
            json: { id },
            headers
        }).json<Answer>();
        if (code !== 200)
...
async function downloadLesson(token: string, id: string, output: string)
{
    ...
    const { code, data, message } = await got.post("https://api.nap.edu.vn/lesson/preview", {
        json: { alias: id },
        headers
    }).json<Lesson>();
    if (code !== 200)
...
export async function download(_: never, token: string, link: string, output: string)
{
    ...
    if (matches = link.match(
        /https:\/\/nap\.edu\.vn\/testing\/(?<id>[0-9a-f]{24})/
    ))
        return downloadExam(token, matches.groups!["id"], output);
    if (matches = link.match(
        /https:\/\/nap\.edu\.vn\/lesson\/(?<id>.+)\/?/
    ))
